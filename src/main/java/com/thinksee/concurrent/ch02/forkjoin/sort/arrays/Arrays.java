 ...
 * @since  1.2
 *
 * 这个类包含各种操作数组的方法（如排序和搜索），还包括一个静态工厂方法，它允许将数组视为列表。
 * 若指定的数组引用为空，这个类都将抛出一个空指针异常。
 * 实现该类的规范就是该类不一定是一个归并排序，但是他必须是稳定的。
 */
public class Arrays {
     ...
     * 并行算法不会进一步划分排序任务的最小数组长度。使用更小的size会导致任务之间的内存争用，从而使得并行变得不可能。
     */
     ...
     * 一个比较器实现了一组相互比较元素的自然排序。当提供的比较器为空可以使用。
     * 为了简化底层实现中的代码共享，compare方法仅为其第二个参数声明类型Object。
     * 注意：TimSort方式使用这个比较器，其他的parallelSort方法基于这个的实现使用相同的比较器
     */
     ...
     * 除了legacyMergeSort之外的排序算法，都是按照1检查2参数扩展为当前包内的私有类的方法实现
     */
     ...
     *
     * 将指定数组进行升序排序。排序算法为双轴快速排序，提供nlogn的时间复杂度，通常比传统的单轴快速排序更快
     ...
     *
     * compareTo: -0.0f < 0.0f，其中Float.NaN大于所有数
     *    <  -0.0f == 0.0f，并且Float.NaN本身无意义（既不大于任何值也不小于任何值）
     * <p>Implementation note: The sorting algorithm is a Dual-Pivot Quicksort
     * by Vladimir Yaroslavskiy, Jon Bentley, and Joshua Bloch. This algorithm
     * offers O(n log(n)) performance on many data sets that cause other
     * quicksorts to degrade to quadratic performance, and is typically
